...
def test_langgraph_agent_handles_one_tool_call_then_final_answer() -> None:
    ...
    trace = run(agent.answer("Which HS code is lithium-ion batteries?"))
...
def test_langgraph_agent_handles_multiple_tools_from_both_servers() -> None:
    fake_openai = FakeOpenAI(
        [
            FakeResponse(
                id="r1",
                output=[
                    FakeFunctionCall(
                        ...
                        json.dumps({"query": "lithium-ion batteries", "limit": 5}),
                        "call-1",
                    )
                ],
            ),
            ...
            FakeResponse(
                ...
                output=[
                    FakeFunctionCall(
                        "function_call",
                        "worldbank__get_development_indicator",
                        json.dumps(
                            {
                                "countries": ["DEU"],
            ...
            FakeResponse(id="r4", output=[], output_text="Trade value plus GDP context."),
        ]
    )
    ...
    trace = run(agent.answer("Compare German battery imports from China with GDP."))
...
def test_langgraph_agent_synthesizes_after_max_iterations() -> None:
    fake_openai = FakeOpenAI(
        [
            FakeResponse(
                id="r1",
            ...
            FakeResponse(id="r2", output=[], output_text="Partial synthesis from observed data."),
        ]
    )
    ...
    trace = run(agent.answer("Keep looking until stopped."))

    assert trace.stopped_reason == "max_iterations"
    assert trace.final_answer == "Partial synthesis from observed data."
    assert "tools" not in fake_openai.responses.calls[1]
    assert fake_openai.responses.calls[1]["previous_response_id"] == "r1"


def test_langgraph_agent_stops_on_unknown_tool_request() -> None:
    fake_openai = FakeOpenAI(
        [
            FakeResponse(
                id="r1",
                output=[FakeFunctionCall("function_call", "missing__tool", "{}", "call-1")],
            )
        ]
    )
    agent = LangGraphAgentLoop(servers={}, model="test-model", client=fake_openai)

    trace = run(agent.answer("Call a missing tool."))

    assert trace.stopped_reason == "tool_request_error"
    assert "Unknown OpenAI tool name" in trace.final_answer


def test_langgraph_agent_stops_on_too_many_tool_calls() -> None:
    fake_openai = FakeOpenAI(
        [
            FakeResponse(
                id="r1",
    ...
    trace = run(agent.answer("Call many tools."))
